...
class TaskProjectTabs(QWidget):
    ...
    def set_userID(self, userID):
        self.controller.set_userID(userID)
        self.tasks_table.controller.set_userID(userID)
        self.projects_table.controller.set_userID(userID)
    ...
    def open_new_task_dialog(self):
        new_task_dialog = NewTaskDialog(self.controller.userID)
        new_task_dialog.exec_()
        self.populate_tasks_table()

    def open_new_project_dialog(self):
        new_project_dialog = NewProjectDialog(self.controller.userID)
    ...
    def open_edit_task_dialog(self):
        ...
        if task_id > -1:
            edit_task_dialog = EditTaskDialog(task_id, self.controller.userID)
    ...
    def open_edit_project_dialog(self):
        ...
        if project_id > -1:
            edit_project_dialog = EditProjectDialog(project_id, self.controller.userID)
    ...
    def mark_task_completed(self):
        ...
        if project_id:
            mark_project_complete_dialog = MarkProjectCompleteDialog(project_id, self.controller.userID)
    ...
    def mark_project_completed(self):
        ...
        if not self.check_project_tasks_completed():
            mark_project_tasks_complete_dialog = MarkProjectTasksCompleteDialog(project_id, self.controller.userID)
            mark_project_tasks_complete_dialog.exec_()
        self.populate_projects_table()

    def open_delete_task_dialog(self):
        task_id = self.tasks_table.get_id()
        delete_task_dialog = DeleteTaskDialog(task_id, self.controller.userID)
        delete_task_dialog.exec_()
        self.populate_tasks_table()

    def open_delete_project_dialog(self):
        project_id = self.projects_table.get_id()
        delete_project_dialog = DeleteProjectDialog(project_id,self.controller.userID)
        delete_project_dialog.exec_()
        self.populate_projects_table()

    def open_project_tasks_dialog(self):
        project_id = self.projects_table.get_id()
        project_tasks_dialog = ProjectTasksDialog(project_id, self.controller.userID)
