...
class TableWidget(QTableWidget):
    ...
    def get_id(self):
        row = self.item(self.currentRow(),0)
        return -1 if row is None else int(row.text())
...
class ProjectTasksTable(TasksTable):
    ...
    def __init__(self, project_id, userID=None):
        super().__init__(userID)
...
class UserTable(TableWidget):
    def __init__(self):
        ...
        self.setHorizontalHeaderLabels(["UserID", "Username", "Password"])
